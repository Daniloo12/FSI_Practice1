...
class PriorityQueue(FIFOQueue):
    ...
    def append(self, item):
        super().append(item)
        self.A.sort(key=lambda elements: elements.path_cost)
...
class PriorityQueueWithSubestimation(FIFOQueue):
    ...
    def append(self, item):
        super().append(item)
        self.A.sort(key=lambda elements:elements.path_cost+self.problem.h(elements))
